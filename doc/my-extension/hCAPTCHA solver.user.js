...
(async function() {
    ...
    function matchImagesUsingTensorFlow(imageUrl, word, i) {
        try {
            ...
            img.src = imageUrl;
            img.onload = () => {
                initializeTensorFlowModel().then(model => model.detect(img)).then(function(predictions) {
        ...
        Jimp.read(imageUrl).then(function(data) {
            data.getBase64(Jimp.AUTO, async function(err, src) {
                ...
                Jimp.read(buffer).then(function(data) {
                    data.crop(0, cropHeight, imageWidth, imageHeight).getBase64(Jimp.AUTO, async function(err, src) {
    ...
    // Reference models: https://github.com/tensorflow/tfjs-models.
    ...
    function unsure(targetNodeText) {
        var targetNode = Array.from(qSelectorAll('div')).find(el => el.textContent === targetNodeText);
        // Works for now.
    ...
    function preProcessImage(base64Image, imageUrl) {
        ...
        Jimp.read(base64Image).then(function(data) {
            data.color([{
                ...
                apply: 'brighten',
                params: [20]
            }]).greyscale().getBase64(Jimp.AUTO, function(err, src) {
                var img = document.createElement("img");
                img.setAttribute("src", src);
    ...
    function preProcessImageMethod4(base64Image, imageUrl) {
        // Resize the image.
        Jimp.read(base64Image).then(function(data) {
            // set JPEG quality and greyscale.
            data.resize(256, Jimp.AUTO).quality(60).greyscale().getBase64(Jimp.AUTO, function(err, src) {
                var img = document.createElement("img");
                img.setAttribute("src", src);
